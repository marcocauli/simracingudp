{
  "permissions": {
    "allow": [
      "WebSearch",
      "WebFetch(domain:ir-core.github.io)",
...
## Commit Ladder (DA SEGUIRE PRIMA DI OGNI COMMIT)

Claude DEVE eseguire questa sequenza PRIMA di proporre un commit:

### Step 1: Analizza le Modifiche
- Leggi `git diff` per capire cosa è cambiato
- Identifica i file modificati e il tipo di cambiamento (feat, fix, test, docs, refactor)

### Step 2: Esegui i Test
```bash
# Backend Java - tutti i test
cd backend && mvn test
...
Se i test falliscono: NON proporre il commit. Fissa prima i test.

### Step 3: Verifica Code Quality
```bash
# Backend Java - formatting e linting
...
Se ci sono problemi di style: correggili PRIMA del commit.

### Step 4: Aggiorna la Documentazione

**SE modificate API routes** → Aggiorna sezione API Endpoints in questo file

**SE aggiunta una nuova feature** → Aggiorna README.md con:
- Nome feature
- Come usarla
- Eventuali nuovi parametri

**SE cambiato comportamento** → Aggiorna le note pertinenti

### Step 5: Proponi il Commit

Solo dopo aver completato tutti gli step precedenti, proponi il commit con:
- Messaggio in conventional commits
- Files specifici da includere (mai `git add -A`)
...
```
feat: add new telemetry packet type
fix: resolve UDP packet parsing error
test: add integration test for telemetry
docs: update README
```
